...
const AddUser = () => {
  const callAddUser = async (userData) => {
    try {
      ...
      toast.error(
        error?.response?.data?.message || "Add failed. Please try again."
      );
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      role: Yup.string()
        .oneOf(["user", "admin", "scanner", "member"], "Invalid user role")
  ...
  return (
    <div className="lg:col-span-3 w-full mx-auto bg-white rounded-lg lg:ml-6 border">
      ...
      <form
        ...
        className="w-full flex flex-col gap-4 mx-auto p-6"
      >
        <div>
          <input
            type="text"
        ...
        <button
          ...
          className={`px-4 py-2 text-white rounded-md transition-all duration-300 ${
            formik.isSubmitting
              ? "bg-gray-400"
              : "bg-base-color hover:bg-green-700"
          }`}
        >
